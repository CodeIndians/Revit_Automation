...
namespace Sheeting_Automation.Source.Tags
{
    public partial class TagCreationForm : Form
    {
        ...
        private List<TagData.TagFormData> CollectFormData()
        {
            List < TagData.TagFormData> formDataList = new List<TagData.TagFormData>();

            // iterate through all the rows 
            foreach(DataGridViewRow row in dataGridView1.Rows)
            {
                // initializr form data struct 
                TagData.TagFormData formData = new TagData.TagFormData();
